        ...
        <>
            <div className="task-for-verification-block">
                <ul className="task-for-verification-list">
                    {TaskForVerification.map(task => <TaskConstructorTasks key={task.id + task.planet_id + task.employee.name} currentTaskId={false} taskId={task.employee.id} blockId={task.id} icon={"/monster-icon.svg"} name={task.employee.name} date={task.updated_at} isCanDelete={false} onClickElement={taskClickHandler}/>)}
                </ul>
