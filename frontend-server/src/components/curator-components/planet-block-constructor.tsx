...
function PlanetBlockConstructor({id, icon, type, date, currentPlanetId, onClickBlock, onDelete}: PlanetBlockProps) {
    const convertedDate = date.split('T')[0].split('-'); 
    const active = currentPlanetId && 'active'
    return ( 
        <li onClick={(evt) => onClickBlock(evt, id)} className={"constructor-planet-list-item " + active}>
