    ...
    if(isDraggable){
        return ( 
            <Draggable draggableId={String(taskId)} index={index}>
                 {(provided) => (
                <li {...provided.draggableProps}  {...provided.dragHandleProps} ref={provided.innerRef} onClick={(evt) => onClickElement(evt, taskId, blockId)} className={"selected-block-monster-list-item " + active}>
                    <img className="profile-logo" src={icon ? '/octopuses/' + icon + '.png' : '/monster-icon.svg'} alt="" width={56} height={56}></img>
